...
const parseRouteID = (rid: string) => {
  const route_dir =
    (new RegExp('[0-9](.*)', 'gm').exec(rid) || ['', ''])[0].split('')[0] ||
    '0';

  const route_branch =
    (new RegExp('[0-9](.*)', 'gm').exec(rid) || ['', ''])[1] || '0';

  let route_family = rid.replace(
    (new RegExp('[0-9](.*)', 'gm').exec(rid) || ['', ''])[0],
    ''
  );
  if (route_family.endsWith('_')) route_family = route_family.slice(0, -1);

  return [route_family, route_branch, route_dir];
};

const Prefix = '[AUSYD:TfNSW/SydneyTrains]';
